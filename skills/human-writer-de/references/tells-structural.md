...
> **German note (human-writer-de).** The header-pyramid detector is purely *structural* (it counts H3 children per H2) and works in German exactly as in EN/FR/ES — it is unaffected by capitalization. But the *header-text* tells (§8 below) differ for German: German capitalizes **all nouns** natively, everywhere, so the English/Spanish "Title Case Every Word" tell is much weaker and harder to spot. In German, a title with every noun capitalized is simply *correct German*, not an AI signature. The German-specific signal is capitalizing the **non-nouns** a German title leaves lowercase — articles, prepositions, conjunctions, verbs, mid-title adjectives: "Wie Man Den Workflow Optimiert" is a tell; "Wie man den Workflow optimiert" is correct. A reviewer must NOT read German noun-capitalization as an AI tell. (Mirrored in `tells-stylistic-de.md` § Capitalized nouns.)
...
Cross-reference `tells-stylistic-de.md` for the vocabulary side. This file flags it as a *structural* tell because the section itself — its presence, placement, and H2 — is the signal.
...
- `tells-stylistic-de.md` — vocabulary side (lexical tells, banned-word lists)
